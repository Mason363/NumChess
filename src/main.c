...
#define BG      0x0000
#define CARD    RGB(0x22, 0x22, 0x25)
...
/* Neutral dark grey — R5=4, G6=8, B5=4 → 8-bit ≈ (33, 32, 33). */
#define SPLASH_BG 0x0000
...
static C buf[900];                       /* row-scratch for screen blending */
#define SCRW 200
#define SCRH 56
static C scratch[SCRW * SCRH];           /* widget composer */

/* ---------------------------------------------------------------- Mixing */

static inline C mix(C a, C b, int t) {
  uint32_t A = (a | (uint32_t)a << 16) & 0x07E0F81F;
  uint32_t B = (b | (uint32_t)b << 16) & 0x07E0F81F;
...
static void fill(int x, int y, int w, int h, C c) {
  if (w > 0 && h > 0)
    eadk_display_push_rect_uniform(
      (eadk_rect_t){(uint16_t)x, (uint16_t)y, (uint16_t)w, (uint16_t)h}, c);
}
...
static void text(const char *s, int x, int y, int big, C fg, C bg) {
  eadk_display_draw_string(s, (eadk_point_t){(uint16_t)x, (uint16_t)y}, big, fg, bg);
}
...
static void rrect(int x, int y, int w, int h, int r, C c) {
  for (int j = 0; j < h; j++) {
    int e = j < r || j >= h - r;
    if (!e) { fill(x, y + j, w, h - 2 * r, c); j = h - r - 1; continue; }
    eadk_rect_t R = {(uint16_t)x, (uint16_t)(y + j), (uint16_t)w, 1};
...
static void stroke(float x0, float y0, float x1, float y1, float w, C c) {
  int bx = (x0 < x1 ? (int)x0 : (int)x1) - (int)w - 1;
  int by = (y0 < y1 ? (int)y0 : (int)y1) - (int)w - 1;
  int bw = (x0 < x1 ? (int)(x1 - x0) : (int)(x0 - x1)) + 2 * (int)w + 3;
  int bh = (y0 < y1 ? (int)(y1 - y0) : (int)(y0 - y1)) + 2 * (int)w + 3;
  float ex = x1 - x0, ey = y1 - y0, l2 = ex * ex + ey * ey;
  if (l2 <= 0) return;
  for (int j = 0; j < bh; j++) {
    eadk_rect_t R = {(uint16_t)bx, (uint16_t)(by + j), (uint16_t)bw, 1};
    eadk_display_pull_rect(R, buf);
    for (int i = 0; i < bw; i++) {
      float px = bx + i + 0.5f - x0, py = by + j + 0.5f - y0;
      float t = clampf((px * ex + py * ey) / l2);
      px -= ex * t; py -= ey * t;
...
static const uint8_t SHADE[8][3] = {
  {0,0,0},{1,1,0},{2,2,0},{0,2,19},{0,2,10},{3,3,0},{1,2,16},{1,3,16}
};
...
static void sprite(int pc, int X, int Y, int small) {
  int s = small ? 15 : 30;
  eadk_rect_t R = {(uint16_t)X, (uint16_t)Y, (uint16_t)s, (uint16_t)s};
  eadk_display_pull_rect(R, buf);
  for (int y = 0; y < s; y++)
    for (int x = 0; x < s; x++) {
      C *o = &buf[y * s + x];
...
static void s_rounded(int w, int h, int r, C c, C behind) {
  for (int j = 0; j < h; j++) {
    ...
    for (int i = 0; i < w; i++) {
      float dx = i < r ? r - i - 0.5f : i >= w - r ? i + 0.5f - (w - r) : 0;
      float d = __builtin_sqrtf(dx * dx + dy * dy) - r;
      row[i] = mix(behind, c, (int)(32 * clampf(0.5f - d)));
    }
  }
}

static void s_rounded_at(int W, int ox, int oy, int w, int h, int r, C c, C behind) {
  for (int j = 0; j < h; j++) {
    float dy = j < r ? r - j - 0.5f : j >= h - r ? j + 0.5f - (h - r) : 0;
    C *row = &scratch[(oy + j) * W + ox];
    for (int i = 0; i < w; i++) {
      float dx = i < r ? r - i - 0.5f : i >= w - r ? i + 0.5f - (w - r) : 0;
      float d = __builtin_sqrtf(dx * dx + dy * dy) - r;
      row[i] = mix(behind, c, (int)(32 * clampf(0.5f - d)));
...
static void s_sprite(int W, int ox, int oy, int pc, int small) {
  int s = small ? 15 : 30;
  for (int y = 0; y < s; y++)
    for (int x = 0; x < s; x++) {
      C *o = &scratch[(oy + y) * W + ox + x];
      if (small) {
        C a = mix(shade(pc, 2*x,   2*y,   *o), shade(pc, 2*x+1, 2*y,   *o), 16);
        C b = mix(shade(pc, 2*x,   2*y+1, *o), shade(pc, 2*x+1, 2*y+1, *o), 16);
        *o = mix(a, b, 16);
...
static void s_push(int X, int Y, int W, int H) {
  eadk_display_wait_for_vblank();
  eadk_display_push_rect((eadk_rect_t){(uint16_t)X, (uint16_t)Y, (uint16_t)W, (uint16_t)H}, scratch);
}
...
static void draw_sq(int s) {
  ...
  for (int y = 0; y < 30; y++)
    for (int x = 0; x < 30; x++) {
      ...
      if (s == cur && sel != -2 && (x < 3 || x > 26 || y < 3 || y > 26))
        c = light ? mix(c, INK, 13) : mix(c, WHITE, 22);
      buf[y * 30 + x] = c;
    }
  eadk_display_push_rect((eadk_rect_t){(uint16_t)X, (uint16_t)Y, 30, 30}, buf);
}

static void draw_board(void) {
  for (int s = 0; s < 120; s++) if (!(s & 0x88)) draw_sq(s);
}
...
static void animate(Move m) {
  ...
  for (int i = 1; i <= n; i++) {
    ...
    int e = 1024 - k * k * k * 1024 / (n * n * n);
    anim_x = x0 + (x1 - x0) * e / 1024;
    anim_y = y0 + (y1 - y0) * e / 1024;
...
static const struct { uint8_t min, inc; } TC[] =
  {{1,0},{3,0},{3,2},{5,0},{10,0},{15,10},{30,0},{0,0}};
...
static void button(int x, int y, int w, int h, const char *s, int on, int icon) {
  C c = on ? GREEN : CARD;
  s_rounded(w, h, 8, c, BG);
  if (icon) {
    int ty = (h - 40) / 2;
    s_rounded_at(w, 8, ty, 40, 40, 9, SQ_L, c);
    if (icon == KING) {
      s_sprite(w, 6,  ty + 5, KING,         0);
      s_sprite(w, 20, ty + 5, KING | BLACK, 0);
    } else {
      s_sprite(w, 13, ty + 5, icon, 0);
    }
  }
  s_push(x, y, w, h);
...
static int list(const char *const *items, const uint8_t *icons, int n,
                int x, int w, int h, int gap, int *i) {
...
static void avatar(int b, int cx, int cy, int r) {
  int s = 2 * r;
  s_rounded(s, s, r, BOTS[b].col, BG);
  s_sprite(s, r - 15, r - 16, BOTS[b].pc, 0);
  s_push(cx - r, cy - r, s, s);
}

static void bot_row(int k, int selected) {
  C c = selected ? CARD : BG;
  s_rounded(148, 20, 6, c, BG);
  s_rounded_at(148, 14, 5, 10, 10, 5, BOTS[k].col, c);
  s_push(6, k * 20, 148, 20);
  text(BOTS[k].name, 32, k * 20 + 3, 0, selected ? WHITE : RGB(0xC8,0xC7,0xC5), c);
  char e[6]; itoa(BOTS[k].elo, e);
  text(e, 146 - 7 * (int)strlen(e), k * 20 + 3, 0, DIM, c);
}

static void bot_info(void) {
  fill(180, 108, 120, 50, BG);
  avatar(bot, 240, 62, 40);
  ctext(BOTS[bot].name, 240, 112, 1, WHITE, BG);
  char e[6]; itoa(BOTS[bot].elo, e);
  ctext(e, 240, 134, 0, DIM, BG);
}

static void side_buttons(void) {
  for (int k = 0; k < 3; k++) {
    int x = 184 + k * 40;
    C c = k == pside ? GREEN : CARD;
    s_rounded(32, 32, 8, c, BG);
    s_sprite(32, 1, 1, KING | (k == 1 ? 16 : k == 2 ? BLACK : 0), 0);
    s_push(x, 172, 32, 32);
  }
}
...
static void overlay_item(const char *const *items, int y, int k, int selected) {
  C c = selected ? GREEN : CARD;
  s_rounded(148, 36, 8, c, CARD);
  s_push(46, y + 6 + k * 36, 148, 36);
...
static void clock_pill(int c, int y) {
  int on = P.side == c;
  C bg = on ? (tm[c] < 10000 && TC[tc].min ? RED : WHITE) : CARD;
  C fg = on ? (bg == RED ? WHITE : INK) : DIM;
  rrect(244, y, 72, 34, 8, bg);
  if (TC[tc].min) {
    char s[8]; fmt_clock(tm[c], s);
...
static int game_over(const char *title, const char *why) {
  ...
  dim(0, 0, 240, 240);
  rrect(20, 58, 200, 124, 12, CARD);
  ctext(title, 120, 74, 1, WHITE, CARD);
  ctext(why, 120, 98, 0, DIM, CARD);
  static const char *const b[2] = {"Rematch", "Menu"};
  int i = 0;
  for (int k = 0; k < 2; k++) {
    C c = k == i ? GREEN : CARD;
    s_rounded(86, 38, 8, c, BG);
    s_push(30 + k * 94, 130, 86, 38);
...
again:
  ...
  for (;;) {
    ...
    if (reason) {
      ...
      static char t[12];
      if (md == M_BOT && winner >= 0 && winner != pcol)
        strcpy(strcpy(t, title) + strlen(title), " won"), title = t;
    ...
    int m = human(e);
    if (m == -2) { undo(md == M_BOT ? (hp >= 2) * 2 : 1); continue; }
    if (m == -1) {
      ...
      if (md == M_BOT && k >= 2) k++;
      if (k == 0) { undo(md == M_BOT ? (hp >= 2) * 2 : 1); continue; }
      if (k == 1) { flip ^= 1; autoflip = flip == P.side; }
      if (k == 2 || k == 3) {
...
void ui_tick(void) {
  ...
  spin++;
  for (int k = 0; k < 8; k++) {
    static const int8_t dx[8] = {0,11,16,11,0,-11,-16,-11};
    static const int8_t dy[8] = {-16,-11,0,11,16,11,0,-11};
    if (spin_mode == 1)
      fill(278 + dx[(spin-k)&7], 118 + dy[(spin-k)&7], 5, 5, mix(BG, WHITE, 32 - k * 4));
    else if (k < 3)
      fill(271 + k * 8, spin_y, 3, 3, (spin % 3) == k ? WHITE : CARD);
  }
}
...
static void pregenerate(int kind) {
  static Undo h[16];
  Puzzle cur = PZ; Pos p = P; int n = hp;
  memcpy(h, H, sizeof h);
...
static void puzzles(int kind) {
  ...
  have_next = 0;
  for (;;) {
    if (have_next) { PZ = next, have_next = 0; }
    else {
      fill(240, 0, 80, 240, BG);
      sel = -2;
      draw_board();
    ...
    for (;;) {
      ...
      if (m == -1) {
        ...
        draw_board();
        if (k == 3) return;
        if (k == 2) { if (!done && !failed) rate(0); break; }
        if (k < 0 || done) continue;
        Move sm = PZ.mate ? (step ? mate_move(rem) : PZ.sol[0]) : PZ.sol[step];
        if (k == 0) { hintsq = MFROM(sm); draw_sq(hintsq); continue; }
...
static void splash(void) {
  /* Fade in: 17 steps × 25 ms = ~425 ms, smooth */
  for (int t = 0; t <= 32; t += 2) {
    splash_frame(t);
    eadk_timing_msleep(25);
  }
  eadk_timing_msleep(550);       /* hold at full brightness */
  /* Fade out */
  for (int t = 32; t >= 0; t -= 2) {
    splash_frame(t);
    eadk_timing_msleep(25);
  }
  fill(0, 0, 320, 240, BG);
